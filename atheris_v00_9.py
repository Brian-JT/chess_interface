...
global MoveInput, MoveCount, MoveList, Side, Root, InputGui, GameArray, BoardCanvas, InputEntry, ContentFrame, \
    GameEnd, CheckmateLabel, attacking_coords, BlockingTargetList, MoveInputList, BoardScalar, OldBoardStates, \
    TotalEngineMs, TotalMoveCalcs
...
def is_checkmate(boardstate: list[list[Tile]], side: bool, checking_piece: str, a_row: int, a_col: int, k_row: int,
                 k_col: int) -> bool:
    # print(1206, f"is checkmate called, checking if the piece at {index_column(a_col)}{a_row + 3} is checkmating the opposing king")
...
def intermediate_layer(boardstate: list[list[Tile]], move_list_list: list[list[str]], side: bool, move_count: int, ) -> list[list[str]]:
    ...
    # How many moves it has to simulate for each possible move
    depth = len(move_list_list[1])
...
def final_layer(boardstate: list[list[Tile]], move_list_list: list[list[str]], side: bool, move_count: int,) -> list[list[str, int]]:
    """Calculates all the legal moves at a certain depth"""
    global TotalMoveCalcs
...
def engine_move(boardstate_real: list[list[Tile]], side: bool) -> str:
    ...
    # Copies the boardstate to make sure the game board is kept separate from simulation boards
    boardstate = copy.deepcopy(boardstate_real)
    ...
    # Second layer
    side = not side
    possible_moves_2 = intermediate_layer(boardstate, possible_moves_1, side, 6400)

    # Third layer
    side = not side
    possible_moves_3 = intermediate_layer(boardstate, possible_moves_2, side, 128000)

    # Fourth layer
    side = not side
    depth_3_evals = final_layer(boardstate, possible_moves_3, side, 1000000)
...
if __name__ == "__main__":
    ...
    test_fen = "r1bqk1nr/pppp1ppp/2nb4/1B2p3/4P3/5N2/PPPP1PPP/RNBQK2R w KQkq"

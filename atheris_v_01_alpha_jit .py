            ...
            for col in range(8):
                if boardstate[row][col].piece != " " and boardstate[row][col].piece.isupper() == side:
                    for coord in has_moves(boardstate, row, col):
                        ...
                        # Makes the suggested move
                        boardstate = simple_update(boardstate, row, col, row_e, col_e)

                        # Turns the boardstate into a numpy array for numba JIT compiler
                        #for
                        boardstate2 = boardstate

                        # Evaluates numpy array
...
if __name__ == "__main__":
    ...
    king_table_mid = np.array(king_table_mid)
    king_table_end = np.array(king_table_end)

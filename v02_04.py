...
def chess_turn(
    ...
    if play_bot_b and not side or play_bot_w and side:
        ...
        move_input = str(
            input(f"Enter player move for {'white' if side else 'black'}:")
        )
...
def play_move(
    ...
    """Checks if the start coords have the legal move to the second coords,
    then plays the move if yes. tuple[turn, end, move pgn list, move count (halfmoves)]"""
...
def calculate(boardstate: list[list[Tile]], side: bool,) -> None:
    pass
...
play_bot_w, play_bot_b = False, True
